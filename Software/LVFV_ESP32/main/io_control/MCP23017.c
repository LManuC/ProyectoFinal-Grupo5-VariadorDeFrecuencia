 ...
 *
 * @brief Función que inicializa el puerto I2C y el mutex para acceder ordenadamente al hardware.
 *
 * @param[in] register_address
 *      Es la dirección de memoria del registro que se quiere acceder en el MCP23017.
 *    
 * @param[in] data
 *      Información que quiere escribirse en @p register_address
 *    
 * @return
...
/**
 * @fn static bool i2c_is_hardware_free();
 *
 * @brief Función que pide recursos de hardware para realizar una operación de lectura o escritura.
 *
 * @details En caso que los recursos de hardware estuviesen siendo usados, duerme la app durante 100ms
 *
 * @attention Función bloqueante
 */
static bool i2c_is_hardware_free();

/**
 * @fn static void i2c_release_hardware();
 *
 * @brief Función que devuelve recursos de hardware luego de realizar una operación para que pueda realizarse una nueva lectura o escritura
 */
static void i2c_release_hardware();

/**
 * @fn static esp_err_t mcp_register_read(uint8_t register_address, uint8_t *data);
 *
 * @brief Función que lee @p register_address del MCP23017. La información obtenida se almacena en @p data
 *
 * @param[in] register_address
 *      Es la dirección de memoria del registro que se quiere acceder en el MCP23017.
 *    
 * @param[out] data
 *      Información que leida de @p register_address
 *    
 * @return
 *      - ESP_OK en caso de éxito.
 *      - ESP_ERR_INVALID_ARG Error de parámetro
...
/**
 * @fn static esp_err_t mcp_register_write(uint8_t register_address, uint8_t data);
 *
 * @brief Función que escribe en @p register_address del MCP23017 el dato @p data
 *
 * @param[in] register_address
 *      Es la dirección de memoria del registro que se quiere acceder en el MCP23017.
 *    
...
static const char *TAG = "MCP23017";                /*!< Varaible que se imprime en los ESP_LOG */
SemaphoreHandle_t xI2C_Mutex = NULL;                /*!< Semáforo (Mutex) que es utilizado para que dos comandos sean ejecutados al mismo tiempo y evitar que uno superponga al otro, trayendo datos incorrectos del MCP23017 */

#define I2C_MASTER_TIMEOUT_MS       100             /*!< Tiempo durante el cual se espera alguna respuesta del esclavo antes de cortar la comunicación */
#define I2C_MASTER_SCL_IO           15              /*!< GPIO para SCL de la comunicación con el MCP23017 */
#define I2C_MASTER_SDA_IO           2               /*!< GPIO para SDA de la comunicación con el MCP23017 */
#define I2C_IO_MASTER_NUM           I2C_NUM_1       /*!< Número de puerto I2C de la comunicación con el MCP23017 */
#define I2C_MASTER_FREQ_HZ          100000          /*!< Frecuencia estándar de operción del I2C para comunicación con el MCP23017 en 100000MHz*/
#define I2C_MASTER_TX_BUF_DISABLE   0               /*!< Largo de buffer I2C en 0 porque no es utilizado al ser un dispositivo maestro el ESP32 */
#define I2C_MASTER_RX_BUF_DISABLE   0               /*!< Largo de buffer I2C en 0 porque no es utilizado al ser un dispositivo maestro el ESP32 */
...
static esp_err_t i2c_master_init(void) {
    ...
    ESP_ERROR_CHECK(i2c_param_config(I2C_IO_MASTER_NUM, &conf));
    ESP_ERROR_CHECK(i2c_driver_install(I2C_IO_MASTER_NUM, conf.mode,
...
static esp_err_t mcp_register_read(uint8_t register_address, uint8_t *data) {
    ...
    i2c_master_stop(cmd);
    ret = i2c_master_cmd_begin(I2C_IO_MASTER_NUM, cmd, pdMS_TO_TICKS(I2C_MASTER_TIMEOUT_MS));
