...
\label{sec:0_objeto}
El presente documento tiene por objeto presentar los diferentes puntos para el ensayo de las pruebas que demuestran que el equipo cumple con sus especificaciones acordadas en el documento de Anteproyecto.

Ejecutando los ensayos detallados en las secciones siguientes, se podrá validar el correcto funcionamiento de las distintas etapas del sistema. Asimismo, se evalúan parámetros eléctricos relevantes tales como tensiones, corrientes, formas de onda, estabilidad del bus DC y comportamiento del sistema ante distintas condiciones de carga.

Los resultados obtenidos permitirán determinar el grado de cumplimiento de los objetivos planteados inicialmente, identificar limitaciones del diseño actual y establecer lineamientos claros para futuras mejoras y optimizaciones del equipo.

\newpage
\section{Identificación del Producto}
\label{sec:1_ident}

El producto bajo ensayo corresponde a un prototipo funcional de variador de frecuencia trifásico alimentado con corriente continua, desarrollado en el marco del presente proyecto de grado y conforme al alcance definido en la etapa de Anteproyecto.

El dispositivo que se ve en la figura~\ref{fig:Dispositivo} se diseñó para accionar motores asincrónicos trifásicos de hasta $1/3 HP$ ($240 W$) de potencia nominal, a partir de una fuente de alimentación de $12 VDC$, generando una salida trifásica de $220 Vac$ con frecuencia variable en el rango de $0 Hz$ a $150 Hz$.
...
El sistema integra una etapa de conversión y elevación de tensión de corriente continua, una etapa inversora trifásica controlada digitalmente, y un sistema de control que permite la variación de la velocidad de giro del motor mediante el ajuste de la frecuencia de salida. El dispositivo contempla señales de control e interfaz típicas del ámbito industrial, tales como entrada analógica de 0–10 V para referencia de velocidad, parada de emergencia y selección de velocidades preconfiguradas mediante entradas digitales.

En cuanto a las capacidades de comunicación, el producto admite la incorporación de conectividad WiFi orientada a la configuración. No obstante, en el alcance de este proyecto dicha funcionalidad se limita a una red local, permitiendo únicamente la configuración del equipo en las cercanías del inversor, sin contemplar operación remota a través de internet.
...
Finalmente, se realizaron ensayos de rendimiento y durabilidad, permitiendo cuantificar la eficiencia de las distintas etapas del sistema y evaluar su capacidad de operación continua bajo carga. 

Todos los ensayos se efectuaron en un banco de pruebas controlado, utilizando instrumentación adecuada y respetando las limitaciones propias del prototipo desarrollado.
...
En el caso del inversor, las mediciones directas de corriente de salida fueron descartadas a causa de la presencia de ruido y resultados no coherentes, consecuencia de la conmutación PWM debiendo utilizar una carga resistiva de valor conmocido durante la evaluación del rendimiento del inversor que permita estimar la potencia entregada a partir de la tensión de línea medida.
...
Se establecieron puntos de medición para la tensión de salida donde marca la figura~\ref{fig:SalidaVAC}, de corriente de entrada con una resistencia de shunt en serie a las fuentes de tensión como se ve en la figura~\ref{fig:DiagramaDeConexionShunt} y la tensión y corriente en el bus DC como marca la figura~\ref{fig:DiagramaDeConexionVIdc}, considerando que la corriente máxima siempre sea menor a 2A.
...
Durante los ensayos funcionales se observó un comportamiento estable del motor en todo el rango de frecuencias ensayado, comprendido entre $10Hz$ y $150Hz$, sin presencia de vibraciones, ruidos anómalos ni inestabilidades, indicando que el sistema trifásico generado por el inversor se encuentra correctamente balanceado y que el esquema de modulación implementado funciona conforme a lo esperado.

Los ensayos de modulación evidenciaron una correcta integridad de las señales generadas, con desfases adecuados entre fases y pulsos simétricos en los tiempos de conmutación. Se verificó en la figura~\ref{fig:v_f-vs-Freq} que el índice de modulación se ajusta correctamente a la estrategia $V/f$, manteniendo una relación proporcional entre la frecuencia de salida y la tensión aplicada al motor con una recta horizontal a lo largo de los diferentes puntos de operación. Esta condición también se puede observar en la figura~\ref{fig:Frecuencia-vs-Tension_AC}, que compara la tensión de salida contra la frecuencia de operación, donde se espera obtener una recta monótonamente creciente.
...
En cuanto al rendimiento, se observó que la etapa inversora presenta una eficiencia elevada $95.2\%$, mientras que la etapa conversora $85.4\%$, la cual constituye la principal fuente de pérdidas del sistema. Finalmente, durante los ensayos de durabilidad, se identificó una limitación térmica en el transformador del conversor, que restringe la potencia continua entregable a aproximadamente $1/6HP$ para una operación prolongada. Se puede ver en las figuras \ref{fig:Corriente} y \ref{fig:Tension} como los parámetros de salida se mantienen estables durante 30 minutos luego del transitorio del arranque.
...
Se detalla en la tabla~\ref{tab:resultados} cual fue el resultado de cada ensayo realizado:
...
\begin{landscape}
    ...
    \hline
    \textbf{Ensayo}	& \textbf{Condición}                & \textbf{Resultado esperado}                                       & \textbf{Resultado obtenido}                   & \textbf{Estado} \\
    \hline
    Funcional       & 1/3HP 50Hz                        & Arranque y parada suave                                           & Mantenimiento de la relación V/f              & PASS \\
    \hline
    Funcional       & 1/3HP 50Hz                        & Arranque programado                                               & Arranque exitoso                              & PASS \\
    \hline
    Funcional       & 1/3HP 50Hz                        & Frenada programada                                                & Detención exitosa                             & PASS \\
    \hline
    Funcional       & 1/3HP 50Hz                        & Cambios de frecuencia de régimen, aceleración y desaceleración    & Parámetros configurados exitosamente          & PASS \\
    \hline
    Funcional       & 1/3HP 50Hz                        & Corte de seguridad por consumo de corriente alto                  & Corte por emergencia                          & PASS \\
    \hline
    Funcional       & 1/3HP 50Hz                        & Corte de seguridad por descenso de tensión excesivo               & Corte por emergencia                          & PASS \\
    \hline
    Funcional       & 1/3HP 50Hz                        & Cambio de velocidad durante el giro por cambio de entrada digital & Cambio de velocidad durante operación         & PASS \\
    \hline
    Funcional       & 1/3HP 50Hz                        & Indicación de frecuencia de operación en salida analógica 0-10V   & Curva V/f lineal en gran rango de operación   & PASS \\
    \hline
    Funcional       & 1/3HP 50Hz                        & Configuración por red Wi-Fi AP                                    & Parámetros configurados exitosamente          & PASS \\
    \hline
    Funcional       & 1/3HP 50Hz                        & Manipulación del motor por red Wi-FI AP                           & Arranque y parada exito                       & PASS \\
    \hline
    Durabilidad     & 1/3HP 10 min - Carga resistivas   & Sin fallas                                                        & Sobrecalientamiento Transformador $T_1$       & FAIL \\
    \hline
    Durabilidad     & 1/6HP 30 min - Carga resistivas   & Sin fallas                                                        & Funcionamiento estable y duradero             & PASS \\
    \hline
    Rendimiento     & 1/6HP 50Hz                        & $\eta_{Inversor} \ge 90\%$                                        & $\eta_{Inversor} \simeq 95.2\%$               & PASS \\
    \hline
    Rendimiento     & 1/3HP 50Hz                        & $\eta_{Convertidor} \ge 80\%$                                     & $\eta_{Convertidor} \simeq 85.4\%$            & PASS \\
    \hline
    \end{tabular}
    \caption{Resultado de las pruebas realizadas}
